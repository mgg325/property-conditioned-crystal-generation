...
def _is_numeric_scalar(value) -> bool:
    if isinstance(value, torch.Tensor):
        return value.ndim == 0 and not value.is_complex() and value.dtype != torch.bool
    ...
    return (
        ...
        and not np.issubdtype(arr.dtype, np.bool_)
...
def collate_mp20_tokens(batch):
    ...
    standard_keys = set(out)
    for key in sorted(set(batch[0]) - standard_keys):
        values = [b.get(key, None) for b in batch]
        if all(_is_numeric_scalar(v) for v in values):
            out[key] = torch.tensor([float(v) for v in values], dtype=torch.float32)
    return out
