...
def generate_sampling_batch(
    ...
    with torch.no_grad():
        for start in range(0, n_samples, chunk_size):
            ...
            sample_property_value = getattr(args, "sample_property_value", None)
            if (
                sample_property_value is not None
                and getattr(ctx.model, "property_embed", None) is not None
            ):
                property_values = torch.full(
                    (end - start,),
                    float(sample_property_value),
                    device=device,
                    dtype=torch.float32,
                )
