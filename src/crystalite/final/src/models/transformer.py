...
class TransformerTrunk(nn.Module):
    ...
    def forward(
        ...
        t_sigma: torch.Tensor | None = None,
        property_film: tuple[torch.Tensor, torch.Tensor] | None = None,
        ...
        if property_film is not None:
            gamma, beta = property_film
            if film_start_layer is None:
                raise RuntimeError("film_start_layer is required when property_film is provided.")
            if gamma.shape != beta.shape:
                raise RuntimeError(
                    f"FiLM gamma/beta shapes must match, got {tuple(gamma.shape)} and "
                    f"{tuple(beta.shape)}."
                )
            expected = (x.shape[0], len(self.blocks) - int(film_start_layer), x.shape[-1])
            if tuple(gamma.shape) != expected:
                raise RuntimeError(
                    f"FiLM gamma/beta must be {expected}, got {tuple(gamma.shape)}."
                )
            if int(film_start_layer) < 0 or int(film_start_layer) >= len(self.blocks):
                raise RuntimeError(
                    f"film_start_layer must be in [0, {len(self.blocks) - 1}], "
                    f"got {film_start_layer}."
                )
        else:
            gamma = beta = None
        ...
        for layer_idx, block in enumerate(self.blocks):
            ...
            if gamma is not None and beta is not None and layer_idx >= int(film_start_layer):
                film_idx = layer_idx - int(film_start_layer)
                x = (1.0 + gamma[:, film_idx, None, :].to(dtype=x.dtype)) * x
                x = x + beta[:, film_idx, None, :].to(dtype=x.dtype)
        x = self.norm_out(x)
        if pad_mask is not None:
            x = x.masked_fill(pad_mask[..., None], 0.0)
