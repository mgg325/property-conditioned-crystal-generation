...
def compute_edm_loss(
    ...
    if sample_weights is not None:
        sample_weights = sample_weights.to(device=device, dtype=sigma.dtype).view(-1)
        if sample_weights.shape[0] != sigma.shape[0]:
            raise ValueError(
                "sample_weights must have one value per batch item: "
                f"got {tuple(sample_weights.shape)} for batch {int(sigma.shape[0])}."
            )
    ...
    def masked_feature_mean(values: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        if sample_weights is not None:
            weighted_mask = mask * sample_weights[:, None, None]
            denom = weighted_mask.sum()
            if values.shape != mask.shape:
                denom = denom * float(values.shape[-1])
            return (values * weighted_mask).sum() / denom.clamp_min(1.0)
        denom = mask.sum()
        if values.shape != mask.shape:
            denom = denom * float(values.shape[-1])
    ...
    if real_mask.any():
        ...
        if coord_loss_mode == "frac_mse":
            ...
            if sample_weights is not None:
                atom_weight = atom_mask * sample_weights[:, None]
                loss_f = (
                    (weight_f_atom[:, None] * dist2_norm) * atom_weight
                ).sum() / atom_weight.sum().clamp_min(1.0)
            else:
                loss_f = (
    ...
    if err_y.numel() > 0 and sample_weights is not None:
        lat_weight = sample_weights[:, None]
        loss_y = ((weight_y * err_y) * lat_weight).sum() / (
            lat_weight.sum() * float(err_y.shape[-1])
        ).clamp_min(1.0)
    else:
...
def compute_edm_loss_per_sample(
    denoised: dict[str, torch.Tensor],
    clean: dict[str, torch.Tensor],
    frac_noisy: torch.Tensor,
    sigma: torch.Tensor,
    ...
    """Per-sample analogue of the EDM loss, used for paired CFG contrast."""
    del frac_noisy
    device = sigma.device
    real_mask = ~pad_mask
    sigma_b = sigma.view(-1, 1, 1)
    sigma_lat = sigma.view(-1, 1)
    bsz = int(sigma.shape[0])
    zero = torch.zeros((bsz,), device=device, dtype=sigma.dtype)
    ...
    atom_mask = real_mask.float()
    mask_exp = atom_mask[..., None]
    atoms_per_sample = atom_mask.sum(dim=1).clamp_min(1.0)
    atoms_per_sample_y = atoms_per_sample

    if real_mask.any():
        err_t = (denoised["type"] - clean["type"]) ** 2
        denom_t = (atoms_per_sample * float(err_t.shape[-1])).clamp_min(1.0)
        loss_t = ((weight_t * err_t) * mask_exp).sum(dim=(1, 2)) / denom_t
        ...
        if coord_loss_mode == "frac_mse":
            err_f = diff**2
            denom_f = (atoms_per_sample * float(err_f.shape[-1])).clamp_min(1.0)
            loss_f = ((weight_f * err_f) * mask_exp).sum(dim=(1, 2)) / denom_f
        elif coord_loss_mode == "cart_metric_vnorm_com":
            mean_diff = (diff * mask_exp).sum(dim=1) / atoms_per_sample[:, None]
            diff_centered = (diff - mean_diff[:, None, :]) * mask_exp
            ...
            dist2_norm = dist2_cart / (scale[:, None] ** 2)
            loss_f = (
                (weight_f[:, 0, 0][:, None] * dist2_norm) * atom_mask
            ).sum(dim=1) / atoms_per_sample
        else:
            raise ValueError(f"Unsupported coord_loss_mode: {coord_loss_mode}")
    else:
        loss_t = zero
    ...
    if lattice_repr.lower() == "y1":
        ...
        err_y = (denoised["lat"] - clean["lat"]) ** 2
    if err_y.numel() > 0:
        loss_y = (weight_y * err_y).mean(dim=1)
    else:
        loss_y = zero
