...
def _guided_denoise_edm(
    *,
    ...
    property_values: torch.Tensor | None,
    ...
    property_values = property_values.to(device=type_noisy.device, dtype=torch.float32)
    if property_values.ndim == 0:
        property_values = property_values.expand(type_noisy.shape[0])
    if property_values.ndim == 2 and property_values.shape[1] == 1:
        property_values = property_values[:, 0]
    if property_values.ndim != 1 or property_values.shape[0] != type_noisy.shape[0]:
        raise ValueError(
            "property_values must have shape (B,) or (B, 1), "
            f"got {tuple(property_values.shape)} for batch size {type_noisy.shape[0]}."
        )

    scale = float(diffusion_guidance_factor)
    force_uncond_mask = torch.ones(
        type_noisy.shape[0], device=type_noisy.device, dtype=torch.bool
    )
    force_cond_mask = torch.zeros_like(force_uncond_mask)

    def run(mask: torch.Tensor) -> dict[str, torch.Tensor]:
        return denoise_edm(
            model=model,
            type_noisy=type_noisy,
            frac_noisy=frac_noisy,
            ...
            force_uncond_mask=mask,
        )

    if abs(scale) < 1e-15:
        return run(force_uncond_mask)
    if abs(scale - 1.0) < 1e-15:
        return run(force_cond_mask)

    denoised_uncond = run(force_uncond_mask)
    denoised_cond = run(force_cond_mask)
    return {
        "type": denoised_uncond["type"]
        + scale * (denoised_cond["type"] - denoised_uncond["type"]),
        "frac": denoised_uncond["frac"]
        + scale * wrap_frac(denoised_cond["frac"] - denoised_uncond["frac"]),
        "lat": denoised_uncond["lat"]
        + scale * (denoised_cond["lat"] - denoised_uncond["lat"]),
...
def edm_sampler(
    ...
    for i, (t_cur, t_next_val) in enumerate(zip(t_steps[:-1], t_steps[1:])):
        ...
        if i < num_steps - 1:
            ...
            denoised_next = _guided_denoise_edm(
                model=model,
